...
def build_model(params: dict, update_params={}) -> StratifiedModel:
    ...
    params["theta"] = (
        params["beta"] * params["rr_reinfection_once_infected"]
    ...
    tb_sir_model.adaptation_functions["universal_death_rateX"] = lambda x: 1.0 / 70

    # #   add  time_variant parameters

    def time_variant_CDR():
        return scale_up_function(
            params["cdr"].keys(), params["cdr"].values(), smoothness=0.2, method=5
        )

    def time_variant_TSR():
        return scale_up_function(
            params["tsr"].keys(), params["tsr"].values(), smoothness=0.2, method=5
        )

    def time_variant_DST():
        return scale_up_function(
            params["dst"].keys(), params["dst"].values(), smoothness=0.2, method=5
        )
    ...
    def my_tv_detection_rate(time):
        return (
            my_tv_CDR(time)
            / (1 - my_tv_CDR(time))
            * (params["gamma"] + params["universal_death_rate"] + params["infect_death"])
        )  # calculating the time varaint detection rate from tv CDR
    ...
    if "strain" in stratify_by:
        ...
        tb_sir_model.stratify(
            ...
            requested_proportions={
                ...
                "rif_R": 0.0,
                "mdr": 0.0,
            },  # adjustment_requests={'tau': tau_adjustment},
            adjustment_requests={"beta": {"ds": 1.0, "inh_R": 0.9, "rif_R": 0.7, "mdr": 0.5}},
        )
        ...
        tb_sir_model.add_transition_flow(
            {
                "type": "standard_flows",
                "parameter": "dr_amplification_inh",
                "origin": "infectiousXstrain_ds",
                "to": "infectiousXstrain_inh_R",
        ...
        def tv_amplification_inh_rate(time):
            return (
                my_tv_detection_rate(time)
                * (1 - my_tv_TSR(time))
                * params["prop_of_failures_developing_inh_R"]
            )
        ...
        tb_sir_model.add_transition_flow(
            {
                "type": "standard_flows",
                "parameter": "dr_amplification_rif",
                "origin": "infectiousXstrain_ds",
                "to": "infectiousXstrain_rif_R",
                "implement": len(tb_sir_model.all_stratifications),
            }
        )
        ...
        def tv_amplification_rif_rate(time):
            return (
                my_tv_detection_rate(time)
                * (1 - my_tv_TSR(time))
                * params["prop_of_failures_developing_rif_R"]
            )
        ...
        tb_sir_model.add_transition_flow(
            {
                "type": "standard_flows",
                "parameter": "dr_amplification_rif",
                "origin": "infectiousXstrain_inh_R",
        ...
        tb_sir_model.add_transition_flow(
            {
                "type": "standard_flows",
                "parameter": "dr_amplification_inh",
                "origin": "infectiousXstrain_rif_R",
                "to": "infectiousXstrain_mdr",
                "implement": len(tb_sir_model.all_stratifications),
            }
        )
