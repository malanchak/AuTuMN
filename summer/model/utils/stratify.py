...
from .stratification_funcs import create_multiplicative_function
...
class Compartment:
    def __init__(
        ...
        assert set(strat_names) == set(strat_values.keys())
        self.name = name
        self.value = value
        self.strat_names = strat_names
        self.strat_values = strat_values

    def stratify(
        self, stratify_name: str, strata_names: List[str], strata_proportions: Dict[str, float]
    ):
        """
        Returns a list of new, stratified compartments to replace the current compartment.
        """
        new_comps = []
        new_strat_names = [*self.strat_names, stratify_name]
        for stratum in strata_names:
            new_value = self.value * strata_proportions[stratum]
            new_strat_values = {**self.strat_values, stratify_name: stratum}
    ...
    def serialize(self) -> Tuple[str, float]:
        """Return string representation of the compartment plus value"""
        strat_strs = [f"{s}_{self.strat_values[s]}" for s in self.strat_names]
        return "X".join([self.name, *strat_strs]), self.value

    @staticmethod
    def deserialize(s: str, value: float):
        ...
        for strat in parts[1:]:
            strat_name, strat_value = strat.split("_")
            strat_names.append(strat_name)
            strat_values[strat_name] = strat_value

        return Compartment(name, value, strat_names, strat_values)

    def __repr__(self) -> str:
        s = self.serialize()
        return f"Compartment<{s}>"
...
def get_stratified_compartments(
    ...
    strata_proportions = get_all_proportions(strata_names, strata_proportions)
    new_comp_names = []
    new_comp_values = []
    for comp_name, comp_value in zip(comp_names, comp_values):
        old_comp = Compartment.deserialize(comp_name, comp_value)
        should_stratify = old_comp.name in comps_to_stratify
        if should_stratify:
            new_comps = old_comp.stratify(stratify_name, strata_names, strata_proportions)
            for new_comp in new_comps:
                new_name, new_value = new_comp.serialize()
                new_comp_names.append(new_name)
                new_comp_values.append(new_value)
        else:
            # Retain non-straitied compartments.
            new_comp_names.append(comp_name)
            new_comp_values.append(comp_value)

    return new_comp_names, new_comp_values
...
def stratify_transition_flows(
    ...
    for n_flow in flow_idxs:
        ...
        stratify_from = find_stem(flow["origin"]) in comps_to_stratify
        stratify_to = find_stem(flow["to"]) in comps_to_stratify
            ...
            for stratum in strata_names:
                ...
                if not adjusted_param_name:
                    ...
                    if not (stratify_from and stratify_to):
                        adjusted_param_name = create_stratified_name(
                            param_name, stratify_name, stratum
                        )
                ...
                if stratify_from:
                    from_compartment = create_stratified_name(
                        flow["origin"], stratify_name, stratum
                    )
                else:
                    from_compartment = flow["origin"]

                if stratify_to:
                    to_compartment = create_stratified_name(flow["to"], stratify_name, stratum)
                else:
                    to_compartment = flow["to"]

                # Add the new flow
                if stratify_name == "strain" and flow["type"] != Flow.STRATA_CHANGE:
                    strain = stratum
                else:
...
def stratify_entry_flows(
    ...
    for stratum in strata_names:
        entry_fraction_name = create_stratified_name("entry_fraction", stratify_name, stratum)
...
def stratify_death_flows(
    ...
    for flow in death_flows:
        ...
        if find_stem(flow["origin"]) in compartments_to_stratify:
            ...
            for stratum in strata_names:
                ...
                new_flow = {
                    ...
                    "origin": create_stratified_name(flow["origin"], stratify_name, stratum),
                    "implement": implement_count,
                }
                new_flows.append(new_flow)
        else:
...
def get_adjusted_parameter(
    ...
    if param_adjust_requests:
        if stratum in param_adjust_requests:
            adjusted_param_name = create_stratified_name(param_name, stratify_name, stratum)
        else:
            adjusted_param_name = param_name
...
def stratify_universal_death_rate(
    ...
    for stratum in strata_names:
        if (
            ...
            stratum_name = create_stratum_name(stratify_name, stratum, joining_string="")
            ...
            if (
                ...
                overwritten_parameter_adjustment_names.append(
                    create_stratified_name("universal_death_rate", stratify_name, stratum)
                )
...
def combine_mixing_matrix(
    old_mixing_categories, old_mixing_matrix, new_mixing_matrix, stratify_name, strata_names
):
    """
                ...
                for n_stratum in range(len(strata_names[:-1])):
                    new_flow = {
                        ...
                        "origin": create_stratified_name(
                            compartment, stratify_name, strata_names[n_stratum],
                        ),
                        "to": create_stratified_name(
                            compartment, stratify_name, strata_names[n_stratum + 1],
                        ),
                        "implement": implement_count,
                    }
                    new_flows.append(new_flow)
